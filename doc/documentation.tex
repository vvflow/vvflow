...
Для вычисления этого интеграла удобно использовать комплексные переменные:
$z = (r_{\gamma x}) - i (r_{\gamma y})$, $z_{1,2} = (c_{1,2~x}) - i (c_{1,2~y})$, $z_s = (r_{sx}) - i (r_{sy}) = \frac{1}{2}(z_1 + z_2)$


\begin{equation}
\label{eq_VortexInfluence}
\Xi_\gamma(\vec c_1, \vec c_2, \vec r_\gamma) = \int\limits_{\vec c_1}^{\vec c_2} [\vec e_z \times \vec K(\vec r,\vec r_\gamma)] \cdot \vec n \cdot dr = 
\begin{cases}
...
\begin{equation}
\label{eq_SourceInfluence}
\Xi_q (\vec c_1, \vec c_2, \vec r_q) = \int\limits_{\vec c_1}^{\vec c_2} \vec K(\vec r, \vec r_q) \cdot \vec n \cdot dr =
\begin{cases}
...
\end{cases}
\end{equation}
Без использования комплексных чисел формуда для ближних источников принимает вид $[\Delta \vec l_{sb} \times (\vec r_{sb} - \vec r_v)]/\varepsilon^2$
