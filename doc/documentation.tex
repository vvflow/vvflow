...
\vec V_\gamma(\vec r) = [\vec e_z \times \vec K(\vec r, \vec r_\gamma)] \cdot \gamma,
\end{equation}
а скорость, индуцированная источником $(\vec r_q, q)$ --- по формуле
\begin{equation}
\label{eq_SourceInfluence}
\vec V_q(\vec r) = \vec K(\vec r, \vec r_q) \cdot q.
\end{equation}
...
\Xi_\gamma(\vec c_1, \vec c_2, \vec r_\gamma) = \int\limits_{\vec c_1}^{\vec c_2} [\vec e_z \times \vec K(\vec r,\vec r_\gamma)] \cdot d \vec r = 
\begin{cases}
...
комплексных чисел:
$(\vec r_{sb} - \vec r_v) \Delta\vec l_{sb} / \varepsilon^2$

%FIXME
Здесь надо дописать формулу для влияния источника (это нужно для СЛАУ и присоединенных источников).
Сейчас вляние вычисляется только для дальних источников, и то плохо (я представляю, что отрезок повернут на $90^{\circ}$ и вычисляю влияние вихря на повернутый отрезок)

\begin{equation}
\Xi_q (\vec c_1, \vec c_2, \vec r_q) = \int\limits_{\vec c_1}^{\vec c_2} \vec K(\vec r, \vec r_q) \cdot d\vec r =
???
\end{equation}
...
$\int (\br V_\text{liquid} \cdot d \br r) = \int (\br V_\text{body} \cdot d \br r)$ \\
Интеграл вычисляется по конткетному отрезку аналитически (см.~\ref{ConvectiveInfluence}). $\br V_\text{liquid}$ --- скорость жидкости, которая включает в себя индуцированную свободными и присоединенными вихрями, а так же скорость на бесконечности. $\br V_\text{body}$ --- скорость самого отрезка.
...
\begin{equation}
\begin{split}
&\int\limits_{\vec c_1}^{\vec c_2} {\vec V(\vec r) d \vec r}
= \int\limits_{\vec c_1}^{\vec c_2} {
	\sum\limits_{s=0}^{N_s} {
		\biggl (
		\gamma_{\att~s} \cdot [\vec e_z \times \vec K(\vec r, \vec r_s)] +
		q_{\att~s}\cdot \vec K(\vec r, \vec r_s)
		\biggr )
	}
	\cdot d \vec r
} = \\ =
...
&\sum\limits_{s=0}^{N_s} {
	...
	\underbrace{
	\int\limits_{\vec c_1}^{\vec c_2} {
		[\vec e_z \times \vec K(\vec r, \vec r_s)] \cdot d \vec r
	}}_{\Xi_\gamma(\vec c_1, \vec c_2, \vec r_s)} + 
	q_{\att~s} \cdot
	\underbrace{
	\int\limits_{\vec c_1}^{\vec c_2} {
		\vec K(\vec r, \vec r_s) \cdot d \vec r
...
присоединенная гамма к
прис источник ку к

Момент сил согласно (\ref{eq12.9}) может быть вычислен по формуле

\begin{equation}
\label{eq_moment}
\begin{array}{l}
 {\rm {\bf M}}=\sum\limits_{k=1}^K {\left( {{\rm {\bf r}}_k -{\rm {\bf r}}_c 
} \right)\times \left( {\Gamma _k \times \left( {{\rm {\bf u}}_k -{\rm {\bf 
V}}_k } \right)} \right)} +\frac{1}{2}\sum\limits_k^K {\left( {{\rm {\bf 
r}}_k -{\rm {\bf r}}_c } \right)^2\frac{\Delta {\rm {\bf \delta }}{ 
}_k}{\Delta t}} 
 -\sum\limits_{k=1}^K {\left( {{\rm {\bf r}}_k -{\rm {\bf r}}_c } 
\right)\times {\rm {\bf V}}_k Q_k } \\ 
\end{array}
\end{equation}
