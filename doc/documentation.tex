...
Теперь разберемся с силами. Для разрешения сопряженности мы применяем второй закон Ньютона, который с учетом всех гидро-динамических сил принимает вид
\begin{equation}
\label{eq_newton}
\vec F_{(\ref{eq_force})} + \left( \dfrac{\rho_b}{\rho_0} - 1 \right) \vec gS - k \Delta \vec R = \dfrac{\vec V_? - \vec V_?(t-\Delta t)}{\Delta t} \dfrac{\rho_b}{\rho_0}S
\end{equation}
