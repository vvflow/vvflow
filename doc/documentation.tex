...
\label{eq_noslip}
\left(\begin{matrix}
\Xi_\gamma(\vec c_1, \vec c_2, \vec r_s)\dotsc& ~~a_{I}& a_{II}& a_{III}\\
\end{matrix}\right)
\cdot X
=
...
Члены $a_1, a_2, a_3$ получаются из влияния присоединенных вихрей (чья интенсивность определяется неизвестной скоростью тела).
...
\end{equation}
%
$\Delta \gamma_s$ --- приращение циркуляции на отрезке за текущий шаг (рожденные минус удаленные). Циркуляция рождается в узлах тела ($\vec c_s$)\\
...
Постеднюю сумму (которая с предыдущего шага), можно посчитать аналитически. Она равна
\begin{equation*}
\dfrac{1}{2} \sum [\vec e_z \times \Delta \vec l_s] V_s^2 = - [\vec e_z \times \vec V_?]\omega_? S + \omega_?^2 (\vec r_\text{com} - (\vec R + \Delta \vec R)) S
\end{equation*}

Итого уравнение превращается в такое:

\begin{equation*}
\vec F_{(\ref{eq_force})} + \left( \dfrac{\rho_b}{\rho_0} - 1 \right) \vec gS - k \Delta \vec R
=
\underbrace{\dfrac{\vec V_? - \vec V_?(t-\Delta t)}{\Delta t} \dfrac{\rho_b}{\rho_0}S}_{m \vec a}
\end{equation*}
...
\left(\begin{matrix}
	...
	\dotsc
	&\abinom{-\frac{S}{\Delta t}\frac{\rho_b}{\rho_0}}{0}
	&\abinom{0}{-\frac{S}{\Delta t}\frac{\rho_b}{\rho_0}}
	&\abinom{0}{0}
\end{matrix}\right)
\cdot X =\\
	...
	+ \underbrace {
			\vec F_\text{dead}
			- \vec F_\tau
			- \dfrac{\rho_b}{\rho_0} S \frac{\vec V_?}{\Delta t}
			+ \frac{1}{2} \sum\limits_{s=1}^{N_s} [\vec e_z \times \Delta \vec l_s] \cdot V_s^2
		}_{\text{с предыдущего шага}}
\end{multline}

Учитывая все сказанное, составим левую часть системы линейных уравнений.
...
-\Delta l_y & \hdotsfor{4} & -\Delta l_y & -\dfrac{\rho_b}{\rho_0}S & 0 & 0\\
\Delta l_x & \hdotsfor{4} & \Delta l_x & 0 & -\dfrac{\rho_b}{\rho_0}S & 0 \\
{r_*^2/2} & \hdotsfor{4} & {r_*^2/2} & 0 & 0 & -\dfrac{\rho_b}{\rho_0}J_c \\
\end{array}
\right)
\cdot
\begin{pmatrix}
...
B_b = 
\begin{pmatrix}
	-\sum\limits_{v=0}^{N_v} \Xi_\gamma(\vec c_1, \vec c_2, \vec r_v) \gamma_v\\
	\vdots \\
	-\sum\limits_{v=0}^{N_v} \Xi_\gamma(\vec c_{s}, \vec c_{s+1}, \vec r_v) \gamma_v\\
	\vdots \\
	-\sum\limits_{v=0}^{N_v} \Xi_\gamma(\vec c_{N_s-1}, \vec c_{N_s}, \vec r_v) \gamma_v\\
	-\sum\limits_{v=0}^{N_v} \gamma_v\\
	\underbrace {
			- \dfrac{\rho_b}{\rho_0} S V_{x?} 
			- \frac{\Delta t}{2} \sum\limits_{s=1}^{N_s} \Delta l_{ys} \cdot V_s^2
			+ F_{x~\text{dead}}
		}_{\text{с предыдущего шага}}
		 - \left(\dfrac{\rho_b}{\rho_0}-1\right)S \Delta t g_x + \Delta t k_x \Delta R_x - F_{\tau x}\\
	\underbrace {
			- \dfrac{\rho_b}{\rho_0} S V_{y?}
			+\frac{\Delta t}{2} \sum\limits_{s=1}^{N_s} \Delta l_{xs} \cdot V_s^2
			+ F_{y~\text{dead}}
		}_{\text{с предыдущего шага}}
		 - \left(\dfrac{\rho_b}{\rho_0}-1\right)S \Delta t g_y + \Delta t k_y \Delta R_y - F_{\tau y}\\
	\underbrace {
			- \dfrac{\rho_b}{\rho_0} J_c \omega_?
			+\frac{\Delta t}{2} \sum\limits_{s=1}^{N_s} (\vec r_* \cdot \Delta \vec l_s) \cdot V_s^2
			+ M_\text{dead}
		}_{\text{с предыдущего шага}}
		 + \Delta t k_\omega \Delta \alpha\\
\end{pmatrix}
\end{equation}

Галя еще дала вот такую формулу, вдруг потом пригодится. Но она неправильная :(
\begin{equation*}
\dfrac{1}{2} \sum [\vec e_z \times \Delta \vec l_s] V_s^2 = - [\vec e_z \times \vec V_?]\omega_? S + \omega_?^2 (\vec r_\text{com} - (\vec R + \Delta \vec R)) S
\end{equation*}
