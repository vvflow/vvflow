...
\item $\br V_{(\text{body}_i)}(t)$, $\omega_{(\text{body}_i)}(t)$ --- Скорость i-го тела. Поступательная и вращательная.
\end{enumerate}

Система координат используется правая. Это значит, если вы рисуете ось $Ox$ слева на право, а ось $Oy$ снизу вверх, то положительным направлением вращения будет являться вращение против часовой стрелки.
...
\end{equation*}
Центр поворота жестко связан с центром системы $Ox_b y_b$. Это нужно учитывать при загрузке тела. Сислема осчета тела используется только при загрузке. Везде внутри комплекса координаты тела записываются в основной системе. 
...
\begin{equation*}
\br r_c = \dfrac{1}{S}\int \br r dS = \dfrac{1}{2S}\int\nabla r^2 dS = -\dfrac{1}{2S}\int \br n r^2 dl = -\dfrac{1}{2S} \sum r^2 [\br e_z \times \Delta \br l]
\end{equation*}

Аналогичный алгоритм можно использовать для момента инерции. Здесь используется теорема Стокса.
\begin{equation*}
J = \rho \int r^2 dS = \dfrac{\rho}{4}\int(\rot([\br e_z \times \br r] r^2) dS = \dfrac{\rho}{4}\int r^2 ([\br e_z \times \br r] \cdot d \br S)
\end{equation*}
