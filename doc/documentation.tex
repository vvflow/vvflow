...
}_{(\vec F_\text{born} + \vec F_\text{att} - \vec F_\text{dead})/\rho_0} -
\underbrace {
...
}_{(M_\text{born} + M_\text{att} - M_\text{dead})/\rho_0} -
\underbrace {
	\frac{1}{2} \sum\limits_{s=1}^{N_s}
...
- \dfrac{\vec F_\text{hydro}}{\rho_0} =&\\
= %=============
%-\omega_? S \cdot [\vec e_z \times \vec V_?]
...
- \dfrac{M_\text{hydro}}{\rho_0} =&\\
= %=============
%- \omega_? S \vec r_{*\text{com}} \cdot \vec V_?
M_\text{dead}&
\end{split}
\end{equation}
...
$\dot{\vec V}_\text{com} = \dot{\vec V}_? + [\vec e_z \times \vec r_{*\text{com}}] \cdot\dot\omega - \omega^2 \vec r_{*\text{com}}$

Уравнение для момента записанов инерциальной системе отсчета, поэтому надо не потерять
$S \rho_b \dot{\vec V} \cdot [\vec e_z \times \vec r_{*\text{com}}]$
...
\label{eq_newton}
\begin{split}
- \dfrac{\rho_b}{\rho_0}S \cdot \vec{\dot V}_?
- \dfrac{\rho_b}{\rho_0} [\vec e_z \times \vec r_{*\text{com}}]S \cdot \dot\omega
+ \dfrac{\vec F_\text{hydro}}{\rho_0}
- \vec F_O & + \sum_{b_\text{child}} \vec F_{bO} = \\
= \dfrac{\rho_b}{\rho_0}S \vec r_{*\text{com}} \cdot \omega^2
-& \left( \dfrac{\rho_b}{\rho_0} - 1 \right)S \cdot \vec g\\
%%%%%%%%%%%%%%%%
- \dfrac{\rho_b}{\rho_0} [\vec e_z \times \vec r_{*\text{com}}]S \cdot \dot{\vec V}_?
