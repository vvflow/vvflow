...
double ConvectiveFast_InfSpeed;
...
int InitConvectiveFast(Space *sS, double sEps, double sInfSpeed)
{
	ConvectiveFast_S = sS;
	ConvectiveFast_Eps = sEps;
	ConvectiveFast_InfSpeed = sInfSpeed;
	return 0;
}
...
void BioSavar(TVortex *Vort, double px, double py, double &resx, double &resy)
{
	...
	multiplier = Vort->g * M_1_2PI / ( drabs2 + ConvectiveFast_Eps ); //FIXME remove 2PI
	resx = -dry * multiplier;
	resy =  drx * multiplier;
}}
...
void SpeedSum(TNode *Node, double px, double py, double &resx, double &resy)
{
	...
	for ( i=0; i<nnlsize; i++ )
	{
		...
		for ( j=0; j<lsize; j++ )
		{
			...
			dry = py - Vort->ry;
			drabs2 = drx*drx + dry*dry;
			multiplier = Vort->g * M_1_2PI / ( drabs2 + ConvectiveFast_Eps ); 
			resx -= dry * multiplier;
			resy += drx * multiplier;
			lVort++;
		}
...
int CalcConvectiveFast()
{
	...
	for ( i=0; i<bnlsize; i++ )
	{
		...
		for ( j=0; j<fnlsize; j++ )
		{
			...
			FuncP1 = FarNode->CMp.g * _1_DistPabs;
			FuncM1 = FarNode->CMm.g * _1_DistMabs;
			FuncP2 = FuncP1 * _1_DistPabs;
			FuncM2 = FuncM1 * _1_DistMabs;
			
			Teilor1 -= M_1_2PI * (FuncP1*DistPy + FuncM1*DistMy);
			Teilor2 += M_1_2PI * (FuncP1*DistPx + FuncM1*DistMx);
			Teilor3 += M_1_PI  * (FuncP2*DistPy*DistPx + FuncM2*DistMy*DistMx);
			Teilor4 += M_1_2PI * (FuncP2 * (DistPy*DistPy - DistPx*DistPx) + FuncM2 * (DistMy*DistMy - DistMx*DistMx));
		}*/
		...
		#define SpeedSumCircle(List) 														\
			...
			{																				\
				...
				obj->vx += Teilor1 + Teilor3*LocaldRx + Teilor4*LocaldRy + SpeedSumResX + ConvectiveFast_InfSpeed;		\
				obj->vy += Teilor2 + Teilor4*LocaldRx + Teilor3*LocaldRy + SpeedSumResY;								\
				lobj++; 																	\
			} 																				
		...
		for ( j=0; j<blsize; j++ )
		{
			...
			for ( k=0; k<nnlsize; k++ )
			{
				...
				for ( m=0; m<olsize; m++ )
				{
					...
					AtanSum += ( atan2( ((1-ScalarMult)*dfi), (1 + ObjRSqr - 2*ScalarMult - SqrOfHalfDfi) )
								-atan2( ((1-ScalarMult*Obj1Rsqr)*dfi), (1 + (1 - 2*ScalarMult)*Obj1Rsqr - SqrOfHalfDfi) )
								) * obj->g; */// FIXME Atan is bad
			...
			bvort->g += ( SpeedSumY*bvortx - (SpeedSumX+2)*bvorty )*dfi;
			//cout << bvort->g << endl;
			lbvort++;
		}
