...
int main(int argc, char *argv[])
{
	...
	double sectors[countfi+1][countr];
	double square[countfi+1][countr];
	...
	fout << "ZONE T=\"Temperature\", I=" << countr << ", J=" << countfi+1 << ", F=POINT\n";
	...
	for(i=0; i<=countfi; i++)
	{
		...
		{
			sectors[i][j] = 0;
			//square[i][j] = ( (1+(j+1)*deltar)*(1+(j+1)*deltar) - (1+j*deltar)*(1+j*deltar) )*M_PI/(countfi+1);		}
			square[i][j] = ( exp(2*j*r0) - exp(2*(j-1)*r0) )*M_PI/(countfi+1);		}
	}
...
//	CountSectors(argv[1],sectors, deltar, countr, countfi);
	for(i=0; i<=countfi; i++)
	{
		for(j=0; j<countr; j++)
		{
			...
			if(i != countfi) 
			{
				/*fout << exp(1+(j+0.5)*deltar)*cos(deltafi*(i+0.5)-M_PI) << "\t" 
					 << exp(1+(j+0.5)*deltar)*sin(deltafi*(i+0.5)-M_PI) << "\t" 
					 << sectors[i][j]/square[i][j] << endl;*/
				 fout << exp((j+0.5)*r0)*cos(deltafi*(i+0.5)-M_PI) << "\t" 
					 << exp((j+0.5)*r0)*sin(deltafi*(i+0.5)-M_PI) << "\t" 
					 << sectors[i][j]/square[i][j] << endl;
			}
			else
			{
				fout << exp((j+0.5)*r0)*cos(deltafi*0.5-M_PI) << "\t" 
					 << exp((j+0.5)*r0)*sin(deltafi*0.5-M_PI) << "\t" 
					 << sectors[0][j]/square[0][j] << endl;
			}
		}
	...
	/*for(j=0; j<countr; j++)
	{
		fout << (1+(j+0.5)*deltar)*cos(deltafi*0.5-M_PI) << "\t" 
			 << (1+(j+0.5)*deltar)*sin(deltafi*0.5-M_PI) << "\t" 
			 << sectors[0][j]/square[0][j] << endl;
	}*/
