...
double Density(Space* S, double px, double py, double precision)
{
	double res=0;
	...
	for (int i=0; i<lsize; i++)
	{
		...
		if (exparg > -8) res+= Vort->g*expdef(exparg);
		Vort++;
	}
...
int main(int argc, char *argv[])
{
	...
	Epsilon(S->VortexList, precision);
	...
	fprintf(pipe, "set palette defined (0 \"blue\", 1 \"white\", 2 \"red\")\n");
	...
	for( double x=xmin; x<=xmax; x+= precision)
	{
		for( double y=ymin; y<=ymax; y+= precision)
		{
			if (x*x+y*y > 1)
			{
				double d= Density(S, x, y, precision);
				if ( d<-0.4) d=-0.4;
